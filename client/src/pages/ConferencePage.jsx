    ...
    <div>
      ...
      {(connected || totalOnline > 0) && (
        <div className="flex items-center gap-3 mb-4">
          <StatPill label="This Room" value={roomOnline} color="var(--green)" />
          <StatPill label="Network" value={totalOnline} color="var(--red)" />
          <StatPill label="Speaking" value={unmutedCount} color="#f59e0b" />
        </div>
      )}
      ...
      <div className="hq-card p-5 min-h-[200px]">
        <div className="flex items-center justify-between mb-4">
          <h3 className="hq-label" style={{ marginBottom: 0 }}>
            {connected ? `Active Callers (${unmutedCount})` : 'Active Callers'}
          </h3>
        ...
        {!connected && (
          <div className="text-center py-10">
            ...
            <p className="text-sm" style={{ color: 'var(--muted)' }}>Waiting for conference connection...</p>
            <p className="text-xs mt-1" style={{ color: 'var(--line)' }}>Your SIP phone will connect you automatically</p>
          </div>
        )}

        {connected && callerIds.length === 0 && (
          <div className="text-center py-8">
            <p className="text-sm" style={{ color: 'var(--muted)' }}>No one is speaking</p>
            <p className="text-xs mt-1" style={{ color: 'var(--line)' }}>{roomOnline} user{roomOnline !== 1 ? 's' : ''} in room, all muted</p>
          </div>
        )}
