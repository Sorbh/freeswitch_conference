...
export default function ConferencePage() {
  ...
  // Web takeover: flag on = this browser has device priority over the Yealink
  const [webTakeover, setWebTakeover] = useState(!!account?.web_takeover);
  const [takeoverBusy, setTakeoverBusy] = useState(false);

  useEffect(() => {
    setWebTakeover(!!account?.web_takeover);
  }, [account?.web_takeover]);

  useEffect(() => {
    window.onHotlineTakeoverState = (active) => setWebTakeover(!!active);
    return () => { delete window.onHotlineTakeoverState; };
  }, []);

  async function handleTakeOver() {
    if (takeoverBusy || !window.hotlineClient?.takeOver) return;
    setTakeoverBusy(true);
    try { await window.hotlineClient.takeOver(); setWebTakeover(true); } catch (e) { console.error('takeover failed:', e.message); }
    setTakeoverBusy(false);
  }

  async function handleRelease() {
    if (takeoverBusy || !window.hotlineClient?.releaseTakeover) return;
    setTakeoverBusy(true);
    try { await window.hotlineClient.releaseTakeover(); setWebTakeover(false); } catch (e) { console.error('release failed:', e.message); }
    setTakeoverBusy(false);
  }
  ...
  return (
    <div>
      ...
      {/* Web takeover: offer takeover while the desk phone has the call; offer release while this browser has priority */}
      {isMonitorMode && !webTakeover && (
        <div className="hq-card mb-4 px-4 py-3 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <PhoneIcon />
            <div className="min-w-0">
              <p className="text-sm font-semibold" style={{ color: 'var(--ink)' }}>{t('conference.takeoverMonitorTitle', 'Your desk phone is on the call')}</p>
              <p className="text-xs mt-0.5" style={{ color: 'var(--muted)' }}>{t('conference.takeoverMonitorBody', 'You are listening in monitor mode. Take over to talk from this browser instead.')}</p>
            </div>
          </div>
          <button onClick={handleTakeOver} disabled={takeoverBusy} className="hq-btn flex-shrink-0 px-4 py-2 text-sm" style={{ background: 'var(--red)', opacity: takeoverBusy ? 0.6 : 1 }}>
            {takeoverBusy ? t('conference.takeoverBusy', 'Switching…') : t('conference.takeOver', 'Take over from browser')}
          </button>
        </div>
      )}
      {webTakeover && (
        <div className="hq-card mb-4 px-4 py-3 flex items-center justify-between gap-3" style={{ border: '1px solid rgba(18,183,106,0.25)' }}>
          <div className="flex items-center gap-3 min-w-0">
            <BrowserIcon />
            <div className="min-w-0">
              <p className="text-sm font-semibold" style={{ color: 'var(--ink)' }}>{t('conference.takeoverActiveTitle', 'Browser has the call')}</p>
              <p className="text-xs mt-0.5" style={{ color: 'var(--muted)' }}>{t('conference.takeoverActiveBody', 'Web takeover is on — your desk phone stays idle until you release.')}</p>
            </div>
          </div>
          <button onClick={handleRelease} disabled={takeoverBusy} className="hq-btn flex-shrink-0 px-4 py-2 text-sm" style={{ background: 'var(--muted)', opacity: takeoverBusy ? 0.6 : 1 }}>
            {takeoverBusy ? t('conference.takeoverBusy', 'Switching…') : t('conference.releaseTakeover', 'Release to phone')}
          </button>
        </div>
      )}
...
function PhoneIcon() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--muted)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="flex-shrink-0">
      <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z" />
    </svg>
  );
}
...
function BrowserIcon() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--green)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="flex-shrink-0">
